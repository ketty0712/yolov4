'''
1. 按照input.txt裡面列出的資料，產生qrcode圖片，並存儲至new_qrcode_images資料夾
...
def push_in_word(width):
    ...
    dirPath = os.path.join(cwd, r'./new_qrcode_images')  # 這邊改成qrcode圖片的解壓縮檔路徑
    p = doc.add_paragraph()
    r = p.add_run()
    ...
    doc.save('/opt/lampp/htdocs/ncue_library/qrcode.docx')  # 存檔


if __name__ == '__main__':

    cwd = os.path.dirname(os.path.abspath(__file__))
    dirPath = os.path.join(cwd, r'./new_qrcode_images')

    # 清空名為'qrcode_images'的文件夹
    for f in os.listdir(dirPath):
        filename = os.path.join(dirPath, f)
        if os.path.isfile(filename):
            try:
    ...
    with open(os.path.join(cwd, r'./input.txt'), "r") as f:
        for content in f.readlines():
            ...
            # generate QR code
            qr_code = np.asarray(generateQrcode(
                content).convert("RGB").convert('L'))

            H, W = qr_code.shape[:2]

            module_sz = int(H / 21)

            canvas = np.full((H*2 + module_sz*7, module_sz *
                             (11+6)), 255, dtype=np.uint8)

            # 黑邊框，寬度 1/2 module_sz
